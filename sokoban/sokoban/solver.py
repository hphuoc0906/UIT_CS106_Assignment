...
def breadthFirstSearch(gameState):
    ...
    temp = []
    ### CODING FROM HERE ###
    while frontier: # khi vẫn còn nút chưa được xét
        node = frontier.pop() # lấy trạng thái mới ra
        node_action = actions.pop() # lấy hành động nút mới ra  
        if isEndState(node[-1][-1]): # Kiểm tra vị trí hiện tại của nút đã là trạng thái đích chưa
            temp += node_action[1:] # Nếu là trạng thái kết thúc thì lưu vào kết quả
            break # Kết thúc tìm kiếm
        if node[-1] not in exploredSet: # Nếu trạng thái hiện tại không tồn tại trước đó thì tiếp tục thăm các nút kề. 
            exploredSet.add(node[-1]) # thêm nút hiện tại vào tập đã thăm
            for action in legalActions(node[-1][0], node[-1][1]): # với mỗi hành động có thể thực thi trong nút hiện tại
                newPosPlayer, newPosBox = updateState(node[-1][0], node[-1][1], action) #lấy vị trí mới của hành động tương ứng
                if isFailed(newPosBox): # Nếu vị trí có khả năng bị thất bại
                    continue # thì bỏ qua
                frontier.appendleft(node + [(newPosPlayer, newPosBox)]) # đưa trạng thái mới vào trong frontier
                actions.appendleft(node_action + [action[-1]]) # đưa hành động tương ứng vào trong actions
    return temp # trả lại kết quả
